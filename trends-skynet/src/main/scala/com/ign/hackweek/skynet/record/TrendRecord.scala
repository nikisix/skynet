...
object TrendRecord extends TrendRecord with MongoMetaRecord[TrendRecord] {
  ...
  def getSnapshotWindow(fromDate: Date): List[TrendRecord] = {
    val query = new BasicDBObject("$gt", fromDate)
    this.findAll(new BasicDBObject(TrendRecord.created.name, query)).toList
  }
}
